...
if (empty($seoOp))
{
	...
	http://localhost/modules/smartshop/seo.php/item.2/can-i-turn-the-ads-off.html
	*/
...
if (! empty($seoOp) && ! empty($seoMap[$seoOp])) 
{
	...
	$newUrl = '/modules/smartshop/' . $seoMap[$seoOp] . '.php';
	...
	switch ($seoOp) {
		...
		case 'print':
		default:
			$_SERVER['REQUEST_URI'] = $newUrl . '?pageid=' . $seoArg;
			$_GET['pageid'] = $seoArg;
	}
